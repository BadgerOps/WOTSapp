...
CI/CD: GitHub Actions (lint, test, preview deploy on PR; tag, release, deploy on merge)
```

## Development Environment

**IMPORTANT: Always use `nix-shell` for Node.js CLI commands in this repo.**

```bash
# Enter the nix-shell before running any node/npm commands
nix-shell
...
- Node.js 22 (default)
- nvm for switching Node versions if needed (`nvm use <version>`)
- Proper PATH configuration for node_modules/.bin
...
3. **CONTRIBUTING.md** - How to contribute (PR process, code style)
4. **handoff.md** - Quick onboarding guide for new contributors
...
**🚧 IN PROGRESS (Phase 3 - Military Features):**
...
The app includes automated screenshot generation for README and documentation.

### Demo Mode
Add `?demo=user` or `?demo=admin` to any URL to view the app with mock data (no Firebase required).

### Generating Screenshots
```bash
npm run screenshots          # All screenshots
...
**IMPORTANT:** After making significant UI changes, ask the user:
> "Should I regenerate the screenshots for the README? Run `npm run screenshots` to update them."

Screenshots are stored in `screenshots/output/` (gitignored). See `screenshots/README.md` for details.
