...
exports.approveRecommendation = onCall(async (request) => {
  ...
  // Get approver's name
  const approverName = userDoc.exists && userDoc.data().displayName
  ...
  const title =
    customTitle ||
    `Uniform #${uniform.number} - ${uniform.name}`
  ...
  const weatherSummary =
    `Today's weather: ${tempDisplay}, ${conditions}. ` +
...
exports.autoPublishPendingRecommendations = onSchedule(
  ...
  async () => {
    const db = getFirestore()
    const now = new Date()
    const cutoffTime = new Date(now.getTime() - AUTO_PUBLISH_DELAY_MINUTES * 60 * 1000)

    console.log(`Checking for pending recommendations older than ${cutoffTime.toISOString()}`)
    ...
    console.log(`Found ${toAutoPublish.length} recommendations to auto-publish`)

    let successCount = 0
    for (const { id: recommendationId, data: recommendation } of toAutoPublish) {
      try {
        // Get uniform details
        const uniformDoc = await db
          .collection('uniforms')
        ...
        if (!uniformDoc.exists) {
          console.log(`Uniform ${recommendation.uniformId} not found, skipping recommendation ${recommendationId}`)
          continue
        }
        ...
        await db.runTransaction(async (transaction) => {
          ...
          if (!recDoc.exists || recDoc.data().status !== 'pending') {
            console.log(`Recommendation ${recommendationId} is no longer pending, skipping`)
            return
          }
        ...
        console.log(`Auto-published recommendation ${recommendationId} as Uniform #${uniform.number}`)
        successCount++
      } catch (error) {
        console.error(`Error auto-publishing recommendation ${recommendationId}:`, error)
      }
    }
