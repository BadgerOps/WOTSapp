...
export default function LibertyApprovalQueue() {
  const { user } = useAuth()
  const { requests, loading, error } = usePendingLibertyRequests()
  const { approveRequest, rejectRequest, loading: actionLoading } = useLibertyApprovalActions()
  ...
  async function handleBulkApprove() {
    ...
    if (!confirm(`Approve ${selectedIds.size} liberty request${selectedIds.size !== 1 ? 's' : ''}?`)) {
      return
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Liberty Approval Queue</h2>
          <p className="text-sm text-gray-600 mt-1">
            Review and approve weekend liberty requests
          </p>
        </div>
        {requests.length > 0 && (
      ...
      {requests.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            All liberty requests have been reviewed.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900">
                Reject Liberty Request{rejectingId ? '' : 's'}
              </h2>
              <p className="text-sm text-gray-600 mt-1">
                {rejectingId
            ...
            <div className="px-6 py-4">
              ...
              <label className="block">
                ...
                <textarea
                  ...
                  placeholder="e.g., 'Duty conflict, cannot approve liberty at this time'"
